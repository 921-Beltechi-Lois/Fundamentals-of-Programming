...
class ActivityServices:
    ...
    def get_statistics_for_busiest_days(self):
        ...
        MyDataStructure.sort(day_remaining_hours, lambda x,y: x[1]<y[1])
        return day_remaining_hours
    ...
    def get_statistics_for_a_given_person(self, given_person_id):
        ...
        MyDataStructure.sort(filter_list, lambda x, y: x.start_date_time > y.start_date_time)
        return filter_list
    ...
    def get_searched_list_by_given_description(self, given_description):
        ...
        for activities in self._repository.get_all():
            ...
            #     filter_list.append(activities) # old filter
            filter_list.append(activities)

        filter_list = MyDataStructure.filter_list(filter_list, lambda x: given_description in x.description)
        if len(filter_list) == 0:
    ...
    def get_searched_list_by_given_date_time(self, given_date):
        """
        ~ .start = .end - activities are all in one day, #date = 1 day
        :param given_date:
        :return:
        """
        filter_list = []
        ...
        filter_list = MyDataStructure.filter_list(filter_list, lambda x: x.start_date_time.strftime('%Y') == given_date.strftime('%Y') and \
                                                  x.start_date_time.strftime('%m') == given_date.strftime('%m') and \
                                                                         x.start_date_time.strftime('%d')== given_date.strftime('%d'))
        if len(filter_list) == 0:
            raise ServiceException("given date does not exist")
        return filter_list

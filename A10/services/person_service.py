...
class PersonServices:
    ...
    def get_searched_list_by_given_type_string(self, given_type_string):
        ...
        if given_type_string.isnumeric():
            filter_list = MyDataStructure.filter_list(filter_list, lambda x: given_type_string == x.phone_number)
        else:
            filter_list = MyDataStructure.filter_list(filter_list, lambda x: given_type_string in x.name)
    ...
    #old method
    def get_searched_list_by_given_type_stringg(self, given_type_string):
        filter_list = []
        for person in self.__repository.get_all():
            if not(given_type_string.isnumeric()):
                filter_list.append(person.name)
            elif given_type_string.isnumeric():
                filter_list.append(person.phone_number)
        return filter_list

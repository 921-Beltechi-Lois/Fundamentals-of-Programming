...
class TestActivity(unittest.TestCase):
    ...
    def test_add(self):
        ...
        newActivity = Activity(3, [123, 124], datetime.datetime(2025, 12, 24, 19), datetime.datetime(2025, 12, 24, 21),
                               'fdas')

        with self.assertRaises(RepoException) as re:
            self.__test_activity_service.add(newActivity)  # todo add failed
        self.assertEqual('Activities must not overlap (user cannot have more than one activity at a given time)',
                         str(re.exception))


        newActivity = Activity(3, [123, 124], datetime.datetime(2028, 12, 24, 19), datetime.datetime(2028, 12, 24, 21),
                               'description')

        with self.assertRaises(RepoException) as re:
            self.__test_activity_service.add(newActivity)  # todo add failed
        self.assertEqual('There is already an activity with this name / description!',
                         str(re.exception))
    ...
    def test_update(self):
        ...
        with self.assertRaises(RepoException) as re:
            ...
            self.__test_activity_service.update(activity_updated)
        self.assertEqual("Could not update any data, given Activity_ID does not exist", str(re.exception)) #todo up failed for NON-existing-ID

        # with self.assertRaises(RepoException) as re:
        #     activity_updated = Activity(1, None, datetime.datetime(2025, 12, 24, 20), datetime.datetime(2025, 12, 24, 23), 'fda3s')
        #     self.__test_activity_service.update(activity_updated)
        # self.assertEqual('Given start_time or end_time already exists in the activity_list!', str(re.exception)) #todo up failed
